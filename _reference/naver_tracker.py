...
APP_VERSION = "8.44"  # 버전 정보의 단일 출처 — firebase_auth.CURRENT_VERSION에 덮어씌워짐
...
_FAKE_LOC_PREFIXES = frozenset([
    ...
    "족모임", "찐내돈", "느좋존", "유명블", "모노레",  # v8.41 발견
    "청첩장", "청풍케", "밤야경",  # v8.42 추가 - 화화돼지, 숲속카페 로그
    "골프헬", "냉온탕", "건식사", "일회권", "사우나포",  # v8.42 추가 - 베럴짐 프로모션 쓰레기
])
...
def _find_tokens_in_kw(kw, locations):
    ...
    """keywordList 항목에서 지역 제거 후 의도 토큰 추출 (사전 기반 + 전체 문자열)"""
...
def generate_highly_relevant_keywords(store_name, category, address, menu_items, official_keywords, nearby_station="", keyword_list=None, log_func=None):
    ...
    for suffix in ["본점", "직영점", "지점", "점"]:
        ...
        if clean_name.endswith(suffix):
            ...
            if loc_match:
                ...
                if len(loc) > 5:  # v8.43: "제천청풍호반" 같은 긴 지점명은 무시
                    break
                ...
                locations.append(loc)  # v8.40: 역 자동 추가 제거
    ...
    for token in addr_tokens:
        ...
        if token.endswith("구") and len(token) > 1:
            ...
            locations.append(token[:-1])               # 청풍면 → 청풍
    ...
    # pcmap 교통정보에서 확인된 nearby_station도 locations에 추가
    ...
    if nearby_station and nearby_station not in locations and len(nearby_station) >= 3:
        ...
        if len(nearby_station) <= 6:
            ...
            for _line in ["신분당선", "수인분당선", "경의중앙선", "경춘선", "경강선", "서해선", "신림선", "우이신설선", "김포골드라인", "용인경전철", "의정부경전철"]:
                _station_clean = _station_clean.replace(_line, "")
            # 1~9호선, 공항철도 등 숫자 노선도 제거
    ...
    # v8.40: 쓰레기 키워드 필터 확장
    _BAD_PATTERNS = re.compile(r'영업중|영업종료|영업시간|\d{3,}|에영업|시에|분에|\d+시\d*분|휴무|정기휴무|임시휴무|특가|이벤트|한정|첫방문|할인쿠폰|프로모션|레귤러|수퍼스페셜|디럭스|스탠다드|스위트룸|싱글룸|더블룸|트윈룸|\d+만원(?!대)|지인소개|기간증정|\d+회(?:추가|증정)|뭉칠수록|혜택최대|\d+회헬스|\d+전문|냉온탕|건식사우나|일회권|사우나포함')
    ...
    # v8.42: 대학교 약칭 화이트리스트 (경상대, 부산대 등 "~대" 형태 인식용)
    ...
    for kw in kw_list:
        ...
        # v8.42: 대학교 약칭(경상대, 부산대 등) + 기존 suffix 인식
        m = re.match(r'^[가-힣]{2,4}(?:역|동|구|대학|산|강|천|계곡|공원)', kw)
        # 대학 약칭 체크: "경상대맛집" → "경상대" 추출
        m_univ = re.match(r'^([가-힣]{2,5}대)', kw)
        if m_univ and m_univ.group(1) in _UNIV_NAMES:
            univ_name = m_univ.group(1)
            if univ_name not in locations:
                locations.append(univ_name)
        ...
        # v8.40: 방식 2~5 제거됨, 대신 랜드마크 전체 검색 추가
        # 산/강/천/계곡/공원/호수 suffix는 keywordList 어디에 있든 추출 (금호강, 용오름계곡 등)
    ...
    kws = list(dict.fromkeys(
        ...
        if len(k) <= 8  # v8.40: 9글자 이상 직접 검색 제외
        and (len(k) > 4 or _has_location(k))
    ...
    seen = set()
    # v8.42: 최종 쓰레기 키워드 필터 (지역+쓰레기토큰 조합도 차단)
    ...
    def _is_garbage_kw(kw):
        ...
        # 8자 이상 토큰인데 알려진 의도 토큰이 아니면 쓰레기 (경상대야식술집, 진주콩국수메밀막국수냉면전문 등)
...
class PremiumMarketingApp:
        ...
        if auth:
            ...
            try:
                ...
                if remote_stores is not None:
                    ...
                    # Firestore에 없으면 로컬 stores를 Firestore에 업로드 (덮어쓰기 방지)
                    ...
                    auth.save_user_stores(self.config.get('stores', []))
    ...
    def setup_ui(self):
        ...
        def _fit_canvas(e):
            ...
            # inner는 항상 실제 콘텐츠 크기 유지, 창보다 크면 스크롤


            new_w = max(e.width, req_w)


            new_h = max(e.height, req_h)


            _canvas.itemconfigure(_cwin, width=new_w, height=new_h)


            _canvas.configure(scrollregion=(0, 0, new_w, new_h))
        ...
        inner.bind("<Configure>", lambda e: _canvas.configure(scrollregion=(0, 0, max(inner.winfo_reqwidth(), _canvas.winfo_width()), max(inner.winfo_reqheight(), _canvas.winfo_height()))))
        ...
        if self.auth and FIREBASE_AVAILABLE:
            ...
            def _logout():
                ...
                cred = _os.path.join(_script_dir, 'auto_login.json')
                ...
                _sp.Popen([sys.executable, _os.path.join(_script_dir, '플레이스마스터_실행.pyw')])





            tk.Button(header_frame, text='로그아웃', command=_logout,





                      font=('Malgun Gothic', 9), bg='#334155', fg='#94a3b8',





                      relief='flat', padx=10, pady=4, cursor='hand2').pack(side='right', padx=15, pady=15)
        ...
        tk.Button(_hist_ctrl, text="🗑️ 기록삭제", font=("Malgun Gothic", 8, "bold"), bg="#ef4444", fg="white", bd=0, padx=7, pady=5, cursor="hand2", command=self.exclude_keyword_dialog).pack(side="left", padx=(0, 6))





        tk.Button(_hist_ctrl, text="🚫 검색금지", font=("Malgun Gothic", 8, "bold"), bg="#f59e0b", fg="white", bd=0, padx=7, pady=5, cursor="hand2", command=self.ban_keyword_dialog).pack(side="left", padx=(0, 6))
        ...
        self._hist_action_bar = tk.Frame(_tab_table, bg="#334155", padx=10, pady=6)


        self._hist_action_label = tk.Label(self._hist_action_bar, text="", font=("Malgun Gothic", 10, "bold"), bg="#334155", fg="#f8fafc")


        self._hist_action_label.pack(side="left", padx=(0, 15))


        self._hist_btn_confirm = tk.Button(self._hist_action_bar, text="✔ 확인", font=("Malgun Gothic", 9, "bold"), bg="#10b981", fg="white", bd=0, padx=12, pady=4, cursor="hand2")


        self._hist_btn_confirm.pack(side="left", padx=(0, 6))


        self._hist_btn_cancel = tk.Button(self._hist_action_bar, text="✖ 취소", font=("Malgun Gothic", 9, "bold"), bg="#64748b", fg="white", bd=0, padx=12, pady=4, cursor="hand2", command=self._cancel_hist_edit_mode)


        self._hist_btn_cancel.pack(side="left")


        # 편집모드 상태


        self._hist_edit_mode = None  # "delete" or "ban"


        self._hist_check_vars = {}
        ...
        self.txt_history = tk.Text(_hist_txt_frame, bg="#0f172a", fg="#f8fafc", font=("Consolas", 9), bd=0, wrap="none",
                                   ...
                                   xscrollcommand=_hist_xsb.set, yscrollcommand=_hist_ysb.set, height=20)
    ...
    def refresh_history_tab(self):
        ...
        entries.sort(key=lambda e: e["date"], reverse=True)  # 최신 날짜가 왼쪽
        ...
        dates = [e["date"][5:] for e in entries]  # MM-DD (최신→과거 순서)
        ...
        def _render_row(kw):
            ...
            if self._hist_edit_mode:


                var = tk.BooleanVar(value=False)


                if self._hist_edit_mode == "ban":


                    banned = self._load_banned_keywords(store_name, mode)


                    var.set(kw in banned)


                self._hist_check_vars[kw] = var


                sel_color = "#ef4444" if self._hist_edit_mode == "delete" else "#f59e0b"


                cb = tk.Checkbutton(self.txt_history, variable=var, bg="#0f172a",


                                   selectcolor=sel_color, activebackground="#0f172a", bd=0, highlightthickness=0)


                self.txt_history.window_create(tk.END, window=cb)
            ...
            for d in dates:
                ...
                _is_last = (d == dates[0])  # dates[0]이 최신
            ...
            if len(dates) >= 2:
                ...
                r1 = _norm(dates[1])  # 이전 날짜 (두 번째로 최신)
        ...
        def _is_new_entry(kw):
            ...
            latest = kw_data.get(dates[0], ("-", False))[0]  # 최신 날짜
            ...
            for d in dates[1:]:  # 최신 제외한 이전 날짜들
        ...
        # v8.42: 섹션 분류 개선
        # 1. 순위권: 최신 날짜에 실제 순위 있음
        # 2. 신규 순위권: 30위밖/미검색→순위권 (히스토리 함께 표시)
        # 3. 30위권 밖: 이전 순위권→최신 30위밖 (연속 2회면 숨김)
        # 4. 미검색: 연속 3회면 삭제
        def _get_latest_rank(kw):
            return kw_rank_map.get(kw, {}).get(dates[0], ("-", False))[0] if dates else "-"

        def _is_ranked(rank_str):
            return rank_str and rank_str not in ("-", "", "분석안함", "미검색") and "밖" not in rank_str

        def _count_consecutive_missing(kw):
            cnt = 0
            for d in dates:
                r = kw_rank_map.get(kw, {}).get(d, ("-", False))[0]
                if r == "미검색":
                    cnt += 1
                else:
                    break
        ...
        def _count_consecutive_out(kw):
            cnt = 0
            for d in dates:
                r = kw_rank_map.get(kw, {}).get(d, ("-", False))[0]
                if r == "30위 밖" or "밖" in str(r):
                    cnt += 1
                else:
                    break
            return cnt
        ...
        # 분류
        ranked_kws = []      # 최신 날짜에 순위권
        dropped_kws = []     # 30위권 밖 (이전 순위권→최신 30위밖)
        missing_kws = []     # 미검색

        for kw in filtered_normal_kws:
            latest = _get_latest_rank(kw)
            if _is_ranked(latest):
                if _is_new_entry(kw):
                    pass  # new_entry_kws에서 처리
                else:
                    ranked_kws.append(kw)
            elif latest == "미검색":
                if _count_consecutive_missing(kw) < 3:
                    missing_kws.append(kw)
            elif "밖" in str(latest):
                if _count_consecutive_out(kw) < 2:
        ...
        # v8.42: 새 섹션 구조 (existing_kws → ranked_kws)
        ...
        if dropped_kws:
            self.txt_history.insert(tk.END, sep, "header")
            self.txt_history.insert(tk.END, "  ⬇️ 30위권 밖\n", "none")
            for kw in dropped_kws:
                _render_row(kw)

        # v8.42: 미검색 섹션
        if missing_kws:
            self.txt_history.insert(tk.END, sep, "header")
            self.txt_history.insert(tk.END, "  ⚠️ 미검색\n", "missing")
    ...
    def exclude_keyword_dialog(self):
        ...
        if not store_name:
            ...
            messagebox.showwarning("알림", "먼저 업체를 선택하세요.")


            return


        self._hist_edit_mode = "delete"


        self._hist_check_vars = {}


        self._hist_action_label.config(text="🗑️ 기록삭제 — 삭제해도 30위 안에 들면 다시 표시됨", fg="#ef4444")


        self._hist_btn_confirm.config(bg="#ef4444", command=self._confirm_delete_keywords)


        self._hist_action_bar.pack(fill="x", side="top", after=self._hist_action_bar.master.winfo_children()[0])
    ...
    def ban_keyword_dialog(self):
        ...
        self._hist_edit_mode = "ban"


        self._hist_check_vars = {}


        self._hist_action_label.config(text="🚫 검색금지 — 앞으로 이 키워드는 기록에 안 올림 (검색은 함)", fg="#f59e0b")


        self._hist_btn_confirm.config(bg="#f59e0b", command=self._confirm_ban_keywords)
    ...
    def _cancel_hist_edit_mode(self):


        """편집 모드 취소"""


        self._hist_edit_mode = None


        self._hist_action_bar.pack_forget()
    ...
    def _confirm_delete_keywords(self):


        """기록삭제 확인"""


        to_delete = [kw for kw, var in self._hist_check_vars.items() if var.get()]


        if not to_delete:


            messagebox.showinfo("알림", "삭제할 키워드를 선택하세요.")


            return


        if not messagebox.askyesno("확인", f"{len(to_delete)}개 키워드를 기록에서 삭제하시겠습니까?\n(다음에 30위 이내 진입하면 다시 표시됨)"):


            return


        store_name = self._hist_edit_store


        mode = self._hist_edit_hist_mode


        hist = load_history()


        for e in hist.get(store_name, []):


            if e.get("mode") == mode:


                e["results"] = [r for r in e["results"] if r["keyword"] not in to_delete]


        save_history(hist)


        self._cancel_hist_edit_mode()


        self.refresh_history_tab()


        messagebox.showinfo("완료", f"{len(to_delete)}개 키워드가 삭제되었습니다.")





    def _confirm_ban_keywords(self):


        """검색금지 확인"""


        new_banned = {kw for kw, var in self._hist_check_vars.items() if var.get()}
        ...
        self._save_banned_keywords(store_name, mode, new_banned)
        ...
        if new_banned:
            ...
            for e in hist.get(store_name, []):
                ...
                if e.get("mode") == mode:
                    ...
                    e["results"] = [r for r in e["results"] if r["keyword"] not in new_banned]
        ...
        messagebox.showinfo("완료", f"{len(new_banned)}개 키워드가 검색금지 설정되었습니다.\n(검색은 진행, 기록에만 안 올림)")





    def _load_banned_keywords(self, store_name, mode):
        ...
        try:
            ...
            path = os.path.join(DATA_DIR, "banned_keywords.json")
            ...
            if os.path.exists(path):
                ...
                return set(data.get(store_name, {}).get(mode, []))
    ...
    def _save_banned_keywords(self, store_name, mode, banned_set):
        ...
        try:
            ...
            data[store_name][mode] = list(banned_set)
    ...
    def _load_excluded_keywords(self, store_hash, mode_key):
        ...
        try:
            ...
            path = os.path.join(DATA_DIR, "excluded_keywords.json")
            ...
            if os.path.exists(path):
                ...
                return set(data.get(store_hash, {}).get(mode_key, []))
    ...
    def _save_excluded_keywords(self, store_hash, mode_key, excluded_set):
        ...
        try:
            ...
            data[store_hash][mode_key] = list(excluded_set)
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                for store in target_stores:
                    ...
                    _store_hash = hashlib.md5(store_name.encode()).hexdigest()[:8]





                    _mode_key = "place" if self.analysis_mode == "place" else "blog"





                    _excluded = self._load_excluded_keywords(_store_hash, _mode_key)
                    ...
                    if _excluded:
                        ...
                        _before = len(all_target_keywords)





                        all_target_keywords = [kw for kw in all_target_keywords if kw not in _excluded]





                        if _before > len(all_target_keywords):





                            self.log(f"    ㄴ 🚫 제외 키워드 {_before - len(all_target_keywords)}개 필터링됨")
                    ...
                    # ── 플레이스 키워드 태스크 (v8.35: map.naver.com + __APOLLO_STATE__) ──
                    ...
                    async def _place_task(idx, keyword):
                        ...
                        try:
                            ...
                            try:
                                ...
                                # map.naver.com 검색 (30위까지 정확 검출, 광고 정확 제외)
                                ...
                                p_url = f"https://map.naver.com/p/search/{encoded_kw}"
                                ...
                                await pg.goto(p_url, wait_until="domcontentloaded", timeout=20000)
                                ...
                                for f in pg.frames:
                                    ...
                                    if 'pcmap' in f.url.lower() or f.name == 'searchIframe':
                                ...
                                if search_frame:
                                    ...
                                    # __APOLLO_STATE__에서 place 데이터 추출
                                    ...
                                    place_data = await search_frame.evaluate(r'''() => {
                                        ...
                                        // v8.42: 업종별 *Summary:ID 패턴 + 기존 PlaceListBusinessesItem 지원
                                        ...
                                        for (const key of Object.keys(state)) {
                                            ...
                                            // v8.42: 업종별 *Summary:숫자ID 패턴 + 기존 PlaceListBusinessesItem 지원
                                            const isOldFormat = key.startsWith('PlaceListBusinessesItem:');
                                            const isSummaryFormat = /^[A-Z][a-zA-Z]+Summary:\d+$/.test(key);
                                            if (!isOldFormat && !isSummaryFormat) continue;
                                            if (key.startsWith('AdSummary:')) continue;
                                            ...
                                            items.push({
                                                ...
                                                isAd: val.isAdvertisement === true || val.adType === 'place',
                                        ...
                                        const organic = items.filter(i => !i.isAd).map(i => i.id);
                                        ...
                                        const ads = items.filter(i => i.isAd).map(i => i.id);
                                    ...
                                    ad_count = len(place_data.get('ads', []))
                                ...
                                if not place_id:
                                    ...
                                    self.root.after(0, lambda kw=keyword, r=rank_num, n=len(p_ids), a=ad_count: self.log(
                                        ...
                                        f"      🔎 [{kw}] 오가닉{n}개, 광고{a}개, rank={r}" + (" → 30위 밖" if r > 30 else "")
                                    ...
                                    self.root.after(0, lambda kw=keyword, n=len(p_ids), a=ad_count: self.log(
                                ...
                                self.root.after(0, lambda kw=keyword, e=str(_e)[:60]: self.log(f"      ⚠️ [{kw}] 오류: {e}"))
                        ...
                        if _custom_kws:
                            ...
                            async def _custom_place_task(c_idx, c_kw):
                                ...
                                try:
                                    ...
                                    c_url = f"https://map.naver.com/p/search/{enc}"
                                    ...
                                    await pg.goto(c_url, wait_until="domcontentloaded", timeout=20000)
                                    ...
                                    if search_frame:
                                        ...
                                        place_data = await search_frame.evaluate(r'''() => {
                                            ...
                                            return { organic: items.filter(i => !i.isAd).map(i => i.id) };
                    ...
                    if not self.stop_requested:
                        ...
                        for _rce in _rc_entries:
                            ...
                            for _rcp in _rce.get("results", []):
                                ...
                                # v8.42: 현재 키워드 로직에서 생성되는 것만 복구 대상
                                _current_kw_set = set(all_target_keywords)
                                if (_rck and _rck in _current_kw_set
                    ...
                    try:
                        ...
                        # 검색금지 키워드 필터링 (검색은 했지만 기록에 안 올림)


                        _banned_kws = self._load_banned_keywords(store_name, _hist_mode)


                        if _banned_kws:


                            _hist_results = [r for r in _hist_results if r["keyword"] not in _banned_kws]
