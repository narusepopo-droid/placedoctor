...
| AG단계 | **관리자 토큰 UI + 구독자 중복정리 + 검색어 컬럼 제거** — ① **토큰 관리 UI**: 상태 필터 드롭다운 → **탭(대기 중/승인됨/거절됨/전체)**. **승인 원클릭**(사유 입력 제거). **거절은 세분화 사유 모달**(9종: 일반단어/지명/오타·깨짐/비사전합성어/브랜드/검색량낮음/중복/업종무관/기타 — approval_reason 저장, 추후 발견로직 개선 신호). 결정된 토큰 **재변경 버튼**(거절됨↔승인됨). ② **구독자 중복 방지 강화**: `subscribe_alarm`이 `phone+store_name`을 **place_id 유무와 무관하게 항상 최종 검사**(휴대폰=place_id없이·PC=place_id있게 신청해 중복나던 끌레르 케이스 차단) + 병합 시 place_id 백필. 운영DB 1회 정리(끌레르 중복행 삭제·베럴짐 place_id 백필). 알림톡은 구독자 1행당 1회라 중복행 제거로 1번만 발송. ③ **대시보드 '검색어' 컬럼 제거**: 온사이트 매장검색어라 매장명과 중복·무의미. **외부 유입 검색어(구글/네이버/ChatGPT)는 referrer에서 검색어를 안 넘겨줘 utm_term 태그 없이는 수집 불가**(업계 공통 한계). ④ **구독자 매장 현황 vs 회원리드 수 차이 규명**: `get_subscriber_stores_status`가 **place_id 있는 구독자만** 표시(순위추적 필수)라, 옛 naver.me/URL 방식으로 신청해 place_id 없는 구독자는 제외됨(버그 아님). 현재는 매장명→드롭다운 선택으로 place_id 항상 확보 → 신규는 전부 정상 기록. ⑤ **테스트 기록 정리**: 개발 중 curl 강제재분석(source NULL) 6건 삭제. ⑥ **발견 필터(v8.46)**: 승인 큐에 쓰레기 후보가 애초에 안 올라오게 `detect_unregistered_tokens`에 3종 필터 추가 — 반복조각(생보종생보종=생보종×2)·사전단어 포함 합성어(랩핑전체부분←랩핑)·사양수식 조각(전체부분/김포전체부분). → 검색량 API를 쓰레기에 낭비 안 함(검색량은 '청소부'가 아니라 신규어 '판단도구'). 신규 원자·복합 서비스어(수제버거세트 등)는 계속 발견. |
