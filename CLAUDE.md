...
`backend/main.py`의 `_HTML`에 인라인으로 포함.

화면 섹션 (위→아래):
1. **헤더** — 그린 sticky, 로고+무료진단 뱃지
2. **입력폼** — 매장명+URL 입력, 진단하기 버튼
3. **결과 (진단 후 노출)**
   - 매장 정보 (업종 뱃지, 매장명, 주소)
   - **종합 게이지** — SVG 원형, 0→실제점수 requestAnimationFrame 애니메이션
   - **4축 카드 그리드** — SEO/콘텐츠/활성도/광고 (점수+진행바+세부항목3개+상태칩)
   - **경쟁사 비교** — 가로 막대 2개 (우리 vs 1위), 격차 강조
   - **키워드 카드** — 순위 뱃지, 노출/미노출 구분, 8개 이상 숨김+더보기
   - **닥터 코멘트** — 점수 기반 자동 생성
   - **버튼 3종** — 카톡 리포트(메인), 홈 화면 추가(PWA), 카톡 공유

**점수 색 규칙**: 70↑ 초록(`#22c55e`) / 40↑ 주황(`#f97316`) / 미만 빨강(`#ef4444`)
**브랜드색**: `#03c75a` (헤더·버튼 전용, 점수색과 구분)

광고축 가중치: SEO 34% + 콘텐츠 30% + 활성도 21% + 광고 15% = 100%
광고 집행 여부 감지 미구현 → 현재 20점(미집행) 고정 플레이스홀더
